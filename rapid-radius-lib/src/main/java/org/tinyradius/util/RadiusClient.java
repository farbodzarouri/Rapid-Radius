...
public class RadiusClient {
    ...
    public void send(RadiusPacket response, int port,RadiusPacket request)
            throws IOException, RadiusException {
        DatagramPacket packetOut = makeResponseDatagramPacket(response, port,request);
        DatagramSocket socket = getSocket();
        socket.send(packetOut);
        if (logger.isInfoEnabled()) {
    ...
    protected DatagramPacket makeResponseDatagramPacket(RadiusPacket packet, int port,RadiusPacket request)
            throws IOException, RadiusException {
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        packet.encodeResponsePacket(bos, getSharedSecret(), request);
        byte[] data = bos.toByteArray();
